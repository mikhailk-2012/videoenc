...
VencHeaderData  sps_pps_data, sps_pps_data2;
char sps_pps_buffer[4096];
...
/*
void yuyv_to_nv16(const unsigned char *src, unsigned char *dest, unsigned char *dest_uv, int width, int height)
{
	int i, j;
	unsigned char *src1;
...
void yuyv_to_nv12(const unsigned char *src, unsigned char *dest, unsigned char *dest_uv, int width, int height)
{
	...
	/* copy the U and V values */
	src++;/* point to V */
	src1 = (unsigned char *)src + width * 2;/* next line */

	vdest = dest_uv;
	dest2=(unsigned short *)vdest ;
	for (i = 0; i < height; i += 2) {
		for (j = 0; j < width; j += 2) {
			//      *vdest++ = ((int) src[0] + src1[0] + 1) / 2;/* U */
			//      *vdest++ = ((int) src[2] + src1[2] + 1) / 2;/* V */
			val2 =  ((int) src[0] + src1[0] + 1) >> 1;        /* U */
			val2 |=(((int) src[2] + src1[2] + 1) >> 1) << 8; /* V */
			*dest2++ = val2;
			src += 4;
			src1 += 4;
		}
...
int main( int argc, char **argv )
{
	...
	if( g_options.input != "-" )
	{
		int  w,h,fmt, fps;
		/* create source */
		venc_cxt->CameraDevice = CreateCamera(mwidth, mheight, g_options.fps);
		...
		/* configure camera */
		if (venc_cxt->CameraDevice->configureCamera(venc_cxt->CameraDevice) <0)
		{
			goto fail_out_free_venc;
		}
		printf("Camera: is YUYV = %d\n",  venc_cxt->CameraDevice->isYUYV );

		getV4L2FormatAndSize(venc_cxt->CameraDevice, &w, &h, &fmt, &fps);
		printf("Camera: Width=%d, Height=%d, Ffps=%d, Pix_Fmt=%d\n", w,h,fps,fmt );
...
*/
	...
	if (venc_cxt->CameraDevice->isTvd)
	{
		h264Param.nCodingMode = VENC_FIELD_CODING;
		printf( "FIELD_CODING%d\n");
	}
	else
	{
	...
	pps_t pps;
	h264srd.ReadSPS(*bs, sps);
	//h264srd.ReadPPS(*bs, pps);
	bitstream_start(&wbs);
	...
	while(!bs->IsEnd())
	{
		bitstream_put_ui(&wbs, bs->ReadU8(), 8);
	}
	delete bs;
	bitstream_end(&wbs);
	memcpy(&sps_pps_buffer[0], wbs.buffer, wbs.bit_offset / 8);

	sps_pps_data2.pBuffer = (unsigned char*)sps_pps_buffer;
	sps_pps_data2.nLength = wbs.bit_offset / 8;

	bs = new CBitStreamReader(sps_pps_data2.pBuffer, sps_pps_data2.nLength);
	h264srd.ReadSPS(*bs, sps2);
	...
	for( size_t j = 0; j < g_options.outFifos.size(); j++ )
	{
		...
		if( !stOut->isNV12() )
		{
			stOut->pushBuffer( (void *)sps_pps_data2.pBuffer, sps_pps_data2.nLength, 0, 0 );
			stOut->setH264Header( sps_pps_data2.pBuffer, sps_pps_data2.nLength );
